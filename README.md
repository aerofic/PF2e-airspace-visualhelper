# PF2e Flying Visual Helper V2 / 0.4.2
...
自动测试覆盖高度排序与过滤、相对高度层与无上限自适应 HUD、PF2e Fly Speed 权限、附近高度差、连续高度曲线、分层亚克力材质、双阴影、footprint 提示、非矩形与多格 Token、Mesh 位置/缩放/透明度及美术 UI 的恢复、原生高度数字零写入、跨模块逐属性后写保护、Z Scatter 双命中区与移动让权、目标格 preview、Secret Token、减少动态效果、高度动画以及 50 个飞行 Token 的共享 ticker。
